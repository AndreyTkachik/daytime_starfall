...
OrdStar::OrdStar(const bool i_underground, const float i_x, const float i_y) :
	...
	walk_animation(CELL_SIZE, "Resources/Images/StarWalk.png", ORDSTAR_WALK_ANIMATION_SPEED)
{
	horizontal_speed = -ORDSTAR_SPEED;

	if (0 == underground)
	{
		texture.loadFromFile("Resources/Images/StarDeath0.png");
	}
	else
	{
		texture.loadFromFile("Resources/Images/2ndStarDeath0.png");

		walk_animation.set_texture_location("Resources/Images/2ndStarWalk.png");
	}
}
...
void OrdStar::die(const unsigned char i_death_type)
{
	...
	case 2:
	{
		if (ORDSTAR_DEATH_DURATION == death_timer)
		{
			...
			if (0 == underground)
			{
				texture.loadFromFile("Resources/Images/StarDeath1.png");
			} else {
				texture.loadFromFile("Resources/Images/2ndStarDeath1.png");
			}
		}
	}
	}
...
void OrdStar::update(const unsigned i_view_x, const std::vector<std::shared_ptr<Enemy>>& i_enemies, const MapManager& i_map_manager, Hero& i_hero)
{
	...
	if (-CELL_SIZE < y && x >= static_cast<int>(i_view_x) - CELL_SIZE - ENTITY_UPDATE_AREA && x < ENTITY_UPDATE_AREA + SCREEN_WIDTH + i_view_x && y < SCREEN_HEIGHT)
	{
		...
		if (0 == no_collision_dying)
		{
			...
			if (0 == get_dead(0))
			{
				...
				if (0 == i_hero.get_dead() && 1 == get_hit_box().intersects(i_hero.get_hit_box()))
				{
					if (0 < i_hero.get_vertical_speed())
					{
						die(1);

						i_hero.set_vertical_speed(0.5f * HERO_JUMP_SPEED);
					}
					else
					{
						i_hero.die(0);
				...
				if (i_hero.get_dead() == 0) {
					for (Bullet& bullet : i_hero.get_bullets_vector()) {
